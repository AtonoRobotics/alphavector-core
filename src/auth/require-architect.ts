...
export function requireArchitect(
  ...
  if (principal === "field") {
    throw new SurfaceViolationError(
      "A field token cannot bind, see, or edit the adapter, provider credentials, routines, mail, deadlines, or connectors",
    );
  }
  if (principal) {
    throw new SurfaceViolationError(
      "Only an Architect credential may write adapter bind, provider credentials, routines, mail, deadlines, or connectors",
    );
  }
