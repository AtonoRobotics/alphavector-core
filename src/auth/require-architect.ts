...
export function requireArchitect(
  ...
  if (principal === "field") {
    throw new SurfaceViolationError(
      "A field token cannot sit in the habitat, issue an architect message, or bind, see, or edit the adapter, provider credentials, routines, mail, deadlines, connectors, skills, brokerage, deploy a tenant, or promote a proposal",
    );
  }
  if (principal) {
    throw new SurfaceViolationError(
      "Only an Architect credential may sit in the habitat, issue an architect message, or write adapter bind, provider credentials, routines, mail, deadlines, connectors, skills, brokerage, deploy a tenant, or promote a proposal",
    );
  }
